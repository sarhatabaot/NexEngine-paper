...
public class MMOItemsHook extends PluginItem<MMOItemTemplate> {
    ...
    public @Nullable MMOItemTemplate getPluginItem() {
        ...
        if (type == null) {
            NexEngine.get().error("[MMOItems] Could not found item type: %s".formatted(itemId[0]));
            return null;
        }
        ...
        if (mmoItemTemplate != null) {
            ...
            NexEngine.get().error("[MMOItems] Could not found item: %s (type: %s)".formatted(itemId[1], itemId[0]));
            return null;
        }
    }
