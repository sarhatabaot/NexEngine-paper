...
public class MMOItemsItem extends PluginItem<MMOItemTemplate> {
    ...
    public MMOItemsItem(final ILogger logger) {
        super(logger);
    }

    @Override public boolean available() {
        return Hooks.hasMMOItems();
    }
    ...
    public @Nullable MMOItemTemplate getPluginItem() {
        ...
        if (type == null) {
            logger().error("[MMOItems] Could not found item type: %s".formatted(itemId[0]));
            return null;
        }
        ...
        if (mmoItemTemplate != null) {
            ...
            logger().error("[MMOItems] Could not found item: %s (type: %s)".formatted(itemId[1], itemId[0]));
            return null;
        }
    }
